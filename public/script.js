...
function createNewChat() {
    ...
    chats[id] = { title: 'Percakapan Baru', messages: [] };
    currentChatId = id;
    renderHistory();
    switchChat(id);
    clearAttachment();
            ...
            contentHtml += `
                <div class="space-y-2">
                    ...
                    <div class="flex space-x-2">
                        <a href="${msg.text}" download="dexone-ai-image.jpg" class="text-xs text-gray-400 hover:text-white flex items-center bg-zinc-900 px-2 py-1 rounded border border-zinc-800"><i class="fa-solid fa-download mr-1"></i> Simpan Gambar</a>
                    </div>
                </div>
            `;
        } else {
...
chatForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (text.toLowerCase().startsWith('/image')) {
            const promptText = text.replace(/\/image/i, '').trim();
            ...
            aiResponse = { role: 'assistant', isAiImage: true, text: data.imageUrl };
        } else {
            const formattedHistory = currentChat.messages.slice(0, -1).map(m => ({
                role: m.role === 'user' ? 'user' : 'model',
                parts: [{ text: m.text }]
            }));

            let filePayload = null;
            if (msgAttachment && msgAttachment.data !== '#') {
                filePayload = { base64: msgAttachment.data.split(',')[1], mimeType: msgAttachment.type };
            }

            const res = await fetch('/api/chat', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ message: text, history: formattedHistory, fileData: filePayload })
            });
            const data = await res.json();
            aiResponse = { role: 'assistant', isAiImage: false, text: data.text };
        }

        currentChat.messages.push(aiResponse);
        renderMessages(currentChatId);
    } catch (error) {
        currentChat.messages.push({ role: 'assistant', isAiImage: false, text: "Maaf, terjadi masalah koneksi ke server DexOne AI." });
        renderMessages(currentChatId);
    }
});
